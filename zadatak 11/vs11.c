...
int brisiListu(CouPosition head);
...
int brisiListu(CouPosition head)
{
    CouPosition temp = head;
    CouPosition zaBrisanje = NULL;

    while (temp->next != NULL)
    {
        zaBrisanje = temp->next;
        brisiStablo(zaBrisanje->grad);
        temp->next = zaBrisanje->next;
        free(zaBrisanje);
    }
